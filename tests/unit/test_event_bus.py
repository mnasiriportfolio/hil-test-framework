...
def test_recorder_collects_and_grades():
    ...
    bus.publish(
        RESULT_TOPIC, suite="S", case="c", check="a", expected="1", measured="1", outcome="PASS"
    )
    bus.publish(
        RESULT_TOPIC, suite="S", case="c", check="b", expected="2", measured="9", outcome="CHECK"
    )
...
def test_recorder_marks_fail():
    ...
    rec = ReportRecorder(bus)
    bus.publish(
        RESULT_TOPIC, suite="S", case="c", check="a", expected="1", measured="9", outcome="FAIL"
    )
